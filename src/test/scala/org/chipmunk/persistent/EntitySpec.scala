...
import org.chipmunk.persistent.TestSchema.Animal
import org.chipmunk.persistent.TestSchema.DogSpeciesName
import org.chipmunk.persistent.TestSchema.Species
...
class EntitySpec extends fixture.FlatSpec with TestSchema with Transactions with InMemoryDb {
  ...
  protected def withFixture(test: OneArgTest) = {
    val dogSpecies = new Species(DogSpeciesName)
